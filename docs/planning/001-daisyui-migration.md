...
|---|------|-------|----------|--------|--------------|
| **Phase 0 — 설치 & 기반** | | | | | |
| 1 | `bun --cwd frontend add -D daisyui` 설치 | frontend | P0 | DONE | — |
| 2 | `app.css`에 `@plugin 'daisyui' { themes: light --default, dark --prefersdark; logs: false; }` 추가 | frontend | P0 | DONE | 1 |
| 3 | `bun --cwd frontend run check` + `bun --cwd frontend run build` 통과, 양 모드 스모크 (rootscrollgutter 시프트 확인) | qa | P0 | DONE | 2 |
| **Phase 1 — 컬러 토큰 전면 이관** | | | | | |
...
| 21 | 최종 스윕: 잔존 레거시 grep 0, 양 모드 × 주요 5화면 시각 검증, `bun --cwd frontend run check`/`bun --cwd frontend run build` | qa | P2 | DONE | 20 |
...
- [ ] `bun --cwd frontend run check` + `bun --cwd frontend run build` 통과
