...
<div class="relative select-none">
	<div
		...
		onemblaInit={onInit}
		class="scrollbar-none overflow-hidden rounded-lg py-1 transition-opacity duration-(--duration-fast) outline-none [-ms-overflow-style:none] focus-visible:outline-3 focus-visible:outline-primary [&::-webkit-scrollbar]:hidden {ready
			? 'opacity-100'
			: 'opacity-0'}"
			...
			{#each ordered as date, i (date)}
				...
				<div class="flex min-w-0 shrink-0 grow-0 basis-28 items-center justify-center">
					<button
						...
						data-date={date}
						onclick={() => pick(date)}
						class="flex min-h-11 w-full items-center justify-center transition-[opacity,transform] duration-(--duration-fast)"
					...
					>
						...
						<span
							class="rounded-full leading-none whitespace-nowrap transition-colors {active
								...
								: 'px-2 py-1 text-[13px] font-medium text-(--color-text-muted)'}"
						>
							{label(date)}
						</span>
					</button>
