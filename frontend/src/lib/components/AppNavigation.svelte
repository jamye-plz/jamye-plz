...
{#if variant === 'rail'}
	<aside
		...
		class="fixed inset-y-0 left-0 z-(--z-dropdown) hidden w-(--rail-width) flex-col border-r border-base-300 bg-(--color-surface-raised) p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] pl-[max(0.75rem,env(safe-area-inset-left))] lg:flex"
		aria-label="주요 탐색"
	>
			...
			{#each items as item (item.path)}
				...
				<a
					...
					class="flex min-h-12 items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium transition-colors hover:bg-base-300/70
						...
						{item.current
						? 'bg-(--color-surface-lilac) font-semibold text-base-content hover:bg-(--color-surface-lilac)'
						: 'text-(--color-text-muted) hover:text-base-content'}"
				>
					<span
